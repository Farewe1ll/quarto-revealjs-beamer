...
CambridgeUS 的三色按**语义层级**设计（这是有意偏离上游的一处，上游是蓝/绿/褐红）：

| block | 颜色 | 来源 | 对比度 |
|:---|:---|:---|:---|
| 警示 `alertblock` | `#cc0000` | 与列表符号同色，全篇唯一的"注意"红 | 5.79:1 |
| 示例 `exampleblock` | `#008000` | `example text`，Beamer 钉死 | 5.05:1 |
| 普通 `block` | `#b45309` 赭橙 | 独立于 `--beamer-structure` | 4.94:1 |

两点设计约束：

1. **普通块标题不跟 `structure` 走**。列表符号和警示块共用 `#cc0000`，红色在全篇只表达"注意"这一件事，普通块退到暖色系中段，不与警示信号竞争。
2. **为什么是赭橙而不是亮橙**：亮橙在浅背景上当文字色物理上不可读 —— `#f76707` 只有 2.99:1、`orange #ffa500` 只有 1.94:1、`yellow` 只有 1.06:1。`#b45309` 是仍能满足 4.5:1 的最浅橙档。若要在浅底上使用真正的亮橙，必须给该 block 加填充才行。

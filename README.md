...
色块**刻意做得又薄、又只把留白放在上方**：行内元素的盒子挂在基线上、高度由**等宽字体自己的** ascent/descent 决定（本机为基线上 22.98px、基线下 7.98px，正文大写高度 20px），所以盒子里天然「下重上轻」。上下留白相等时盒子上沿只比正文大写线高 5.1px、下沿却落到基线下 9.4px，读起来就是「方框下坠」；但如果为了居中把留白全加到上方（试过 0.21em），盒子会涨到 36.1px——行高只有 38.4px，列表折行后相邻两行的色块只剩 2.3px 间距，视觉上连成一整块，反而更糟。最终取值是两头的折中，也是这套样式的**设计边界**：`padding: 0.13em 0.2em 0` 把盒子压到 32.2px（相邻两行留 6.2px），同时把下坠压到 1.2px（大写线上方 6.0px、基线下 7.2px，改前是 5.1px / 9.4px）。另外，**中文（日文、韩文）文档里行内代码会整体上移 `0.1em`**：共用一个基线并不够——汉字的墨迹比拉丁字母高得多（实测同一行 27px 对 18px），两者都坐在基线上时，拉丁字母那截墨迹的**重心比汉字低 2.5px**，看起来就是「英文掉下去了」。上移后两者重心对齐（实测差 0.00px），色块本身在这类行里也顺带正了（上 3.6px、下 3.6px）；纯拉丁文档不上移，代码严格坐在基线上，所以这条规则按文档语言用 `:lang(zh/ja/ko)` 限定，跟着 front matter 的 `lang:` 走。原来的 `border` 换成 `box-shadow` 画的 1px 描边：在 block 与表格的浅色底上一样能界定色块，却**不占盒子高度**（省 2px）；再加 `box-decoration-break: clone`，折行的代码得到两个各自完整的薄色块，而不是一块跨行灰板。

...
| 警示 `alertblock` | 黄字 `#ffcd00`，无填充，字重 700 | 有意选择（见下） | **1.48:1** |

三种 block 都**无填充**，只靠文字颜色区分；警示块额外用 700 字重（其余 650）拉开权重。

> **⚠️ 关于警示黄的对比度**：`#ffcd00` 对浅色页面背景实测 **1.48:1**，远低于正文所需的 4.5:1，视觉上偏"淡金"而不是醒目的警示色。这是明确的设计选择。若在某台设备/投影上偏色导致难以辨认，可改回 `--beamer-alert`（红 `#cc0000`）或换成黄底深色字（`#ffd700` 底 + `#1a1a1a` 字，12.4:1）。

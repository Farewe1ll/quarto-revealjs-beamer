...
| 警示 `alertblock` | 黄字 `#ffcd00` + **黑色描边** `4px`，无填充，字重 700 | 黄 + 黑边（见下） | 描边 20.7:1 |

三种 block 都**无填充**；警示块额外用 700 字重（其余 650）与黑色描边拉开权重。

**黄色为什么要加黑边**：`#ffcd00` 自身对浅色页面只有 **1.48:1**，远低于正文所需的 4.5:1，单靠黄色会发白难辨。加上 4px 黑边后，每个字形获得一圈 **20.7:1** 的深色轮廓，字身仍是你要的黄色。实现用 `-webkit-text-stroke` 配合 `paint-order: stroke fill`——后者是把描边画在**字身之后**，否则描边会居中于字形轮廓、把黄色吃掉。

描边宽度是在**真实演示尺寸**下扫掠 2/3/4/5px 比出来的：2px 与 3px 相对普通块显得畏缩，5px 开始像一条黑杠且黄色变薄，**4px 是仍能清晰露出黄字身的最粗值**。可用 `--beamer-block-alert-title-stroke` 调整。

...
色块**刻意做得又薄、又只把留白放在上方**：行内元素的盒子挂在基线上，高度**就是等宽字体自己的字体盒**——本机实测 29px（= 1.19em），而正文大写高度只有 20px，也就是说光字体盒就已经是大写高度的 1.45 倍；`line-height` 对它无效（实测 1 / 1.1 / 1.5 三个取值下盒高不变，所以这个 29px 就是下限）。上下留白相等时盒子上沿只比正文大写线高 5.1px、下沿却落到基线下 9.4px，读起来就是「方框下坠」；但如果为了居中把留白全加到上方（试过 0.21em），盒子会涨到 36.1px——行高只有 38.4px，列表折行后相邻两行的色块只剩 2.3px 间距，视觉上连成一整块，反而更糟。

最终取值是两头的折中，也是这套样式的**设计边界**：`padding: 0.13em 0.2em 0` 把盒子压到 32.2px（拉丁文档里相邻两行留 6.2px），同时把下坠压到 1.2px（大写线上方 6.0px、基线下 7.2px，改前是 5.1px / 9.4px）。

另外，**中文（日文、韩文）文档里行内代码会整体上移 `0.1em`**：共用一个基线并不够——汉字的墨迹比拉丁字母高得多（实测同一行 27px 对 18px），两者都坐在基线上时，拉丁字母那截墨迹的**重心比汉字低 2.5px**，看起来就是「英文掉下去了」。上移后两者重心对齐（实测差 0.00px；0.1em 是在本机字体上标定的，中日韩字形在不同平台回退到不同系统字体，所以测试给了 2px 容差），色块本身在这类行里也顺带正了（上 3.6px、下 3.6px）；纯拉丁文档不上移，代码严格坐在基线上，所以这条规则按文档语言用 `:lang(zh/ja/ko)` 限定，跟着 front matter 的 `lang:` 走。

原来的 `border` 换成 `box-shadow` **内**描边（颜色 `$code-border`，默认 `#d9dde2`）：在 block 与表格的浅色底上一样能界定色块，却既不占盒子高度、也不往盒子外再吃 1px 净空；再加 `box-decoration-break: clone`，折行的代码得到两个各自完整的薄色块，而不是一块跨行灰板。中文文档里还会再去掉上方那点留白——上移 0.1em 已经在负责居中（实测重心差 0.00px），盒子因此回到字体自身的 29px（行内元素的下限，实测 `line-height` 无法再压），于是**上下两行色块之间的净空从看起来的 4.2px 变成 9.4px**。
...
截图与 `tests/baselines/` 中受版本控制的基线比较（容差 1.5% 像素）；实际截图、差异图与 PDF 写入 `tests/_artifacts/`，该目录不进版本控制。

**行内代码的两条像素断言刻意不走基线**：CI 的 ubuntu runner 不装任何中日韩字体（镜像里只有 `fonts-noto-color-emoji`），含汉字的基线与本机渲染必然有差异，所以它们直接从 CDP 取图，比较**同一次截图内部**的两段墨迹（代码 vs 汉字）和**同一页面的两种状态**（当前 padding vs 旧对称 padding、上移 vs 不上移）；断言的是**方向**（修正后是否更平衡、上移是否真的拉近了重心），绝对量值则写在文档里。之所以非用像素不可：用 canvas 推算基线在这一页会差 1.4px，而那个差值指标会把这误差放大一倍，恰好足以放过要抓的那个下沉。

`testPaletteAlgebra` 从**上游主题源码**推导期望色值再与浏览器实测比对——只靠截图做不到这一点，因为基线由本实现自身生成，发现不了自洽的错误。
